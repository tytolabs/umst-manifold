...
fn helmholtz_stationary<B: Backend<FloatElem = f32>>(
    ...
    // For rectil. grid, \(\lambda_{\max}(L_{\mathrm{std}})\leq 8\) is a safe bound; keep \(\omega < 2/(1+s\lambda)\).
    let lambda_upper = 8.0_f32;
    let omega = (1.7 / (1.0 + scale * lambda_upper)).clamp(0.015, 0.22);
