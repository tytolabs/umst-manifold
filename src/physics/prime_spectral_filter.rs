...
impl PrimeSpectralFilter {
    ...
    /// Apply explicit weight table (Track 4 matched-random control).
    pub fn apply_weights<B: Backend<FloatElem = f32>>(
        rho: Tensor<B, 3>,
        weights: &[f32],
        n: usize,
    ) -> Tensor<B, 3> {
        let device = rho.device();
        let w = Tensor::<B, 1>::from_floats(weights, &device).reshape([1, n, 1]);
        rho.mul(w)
    }
}
...
/// Matched-random control: same support as von Mangoldt impulses, random magnitudes (Track 4).
#[must_use]
pub fn matched_random_weight_table(seed: u64, n: usize, epsilon: f32) -> Vec<f32> {
    let mut state = seed;
    (0..n)
        .map(|i| {
            let idx = (i + 1) as u32;
            let lambda = von_mangoldt_weight(idx);
            if lambda <= 0.0 {
                return 1.0;
            }
            state = state.wrapping_mul(6364136223846793005).wrapping_add(1);
            let u = (state >> 32) as f32 / u32::MAX as f32;
            let raw = 1.0 + epsilon * (u * 2.0 - 1.0);
            raw.clamp(1.0 - epsilon, 1.0 + epsilon)
        })
        .collect()
}
