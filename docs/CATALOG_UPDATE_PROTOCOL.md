...
4. When a **primary** fiber pin digest changes (`umst-formal` or `umst-formal-double-slit`), also update `composed_primary_fiber_fingerprint_hex` and `composed_catalog_digest_hex` together. CI enforces coupling via `scripts/catalog_lock_verify.py` (preview / `umst-ucrs` Track F pins do not affect the fingerprint).
