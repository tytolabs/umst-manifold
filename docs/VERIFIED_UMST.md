...
**Status:** Witness spike (`math-verified-umst-typestate`, Wave 6 slot 2).  
**Scope:** `core::verified_umst` staging only — no hot-path or gateway migration.
...
| [`VerifiedUMST<B>`](../src/core/verified_umst.rs) | Staging product of the above |

**Not** the proof-carrying gateway type [`core::tensors::VerifiedUMST<B, P>`](../src/core/tensors.rs)
(Clausius–Duhem witness). The staging struct lives in `verified_umst` until a later wire pass lifts
validated topology into the full UMST carrier.

## API (`core::verified_umst::VerifiedUMST`)
...
cargo test verified_umst
```

- Unit tests in [`src/core/verified_umst.rs`](../src/core/verified_umst.rs) — assemble + reject paths.
- Per-component witnesses in [`src/core/dec_typestate.rs`](../src/core/dec_typestate.rs).
