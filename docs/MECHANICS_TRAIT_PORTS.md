...
**Rust module:** `src/physics/mechanics/port.rs` (feature `mechanics-adjoint`)  
...
| [`MechanicsSolvePort`](../src/physics/mechanics/port.rs) | `mechanics::port` | `(u, σ, Report)` → [`SolveReport`](../src/solve_report.rs) | Port boundary for adjoint / gate / CI consumers |
...
| --- | --- | --- | --- | --- |
| **Bar** | [`BarNetworkMechanicsSolvePort`](../src/physics/mechanics/port.rs) | `F64AdjointBarPcg` | `mechanics-adjoint` | [`VectorMechanicsSolver::solve_equilibrium_with_pcg_report`](../src/physics/mechanics/mod.rs) |
...
pub trait MechanicsSolvePort<B: Backend<FloatElem = f32>> {
    /// Lane telemetry — lifts to [`SolveReport`] via [`ReportedSolve`].
...
Lift `Report` with [`ReportedSolve::into_solve_report`](../src/solve_report.rs).
...
| `mechanics-adjoint` | `mechanics::port` module + `BarNetworkMechanicsSolvePort` |
| `mechanics-adjoint-q1-hex` | Q1 hex elasticity + planned `Q1HexMechanicsSolvePort` |

```bash
export PATH="$HOME/.cargo/bin:$PATH"
cargo test -p umst-manifold --features mechanics-adjoint mechanics_solve_port_associated_report_compiles
```
