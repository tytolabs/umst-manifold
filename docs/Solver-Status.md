...
- **Hardware perf (Q1 hex):** reusable PCG workspace (`solver_region.rs`), warm-start / op-cache knobs, host `DeviceSheet`, fused Krylov reductions, grid catalog witnesses, cockpit iteration budget — verified by `hardware_perf_adversarial` (5 tests) on PR CI. **Not a throughput win** at Striatus mesh sizes: warm-start regressed; block-Jacobi ties Jacobi; geometric and semicoarsening MG fail equilibrium tolerance. Long preconditioner A/B: `q1_hex_perf_levers_ab` (local / release only, ~minutes). Next earned step: **AMG** (USER-gated).
