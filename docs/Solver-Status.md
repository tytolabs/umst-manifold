...
| 11 | Prime-spectral NTT exact density filter (branch-only) | **25** | **Parked** on `prime-spectral-research` — zero mod-q conservation drift validated; L∞ float parity blocked on requantization. Not on `main`; see [issue #26](https://github.com/tytolabs/umst-manifold/issues/26) and MaOS [`FINAL_FINDING.md`](https://github.com/tytolabs/tyto-workspace/blob/prime-spectral-research/outputs/prime-spectral-research/FINAL_FINDING.md). |
