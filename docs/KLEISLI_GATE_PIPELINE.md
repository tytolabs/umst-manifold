...
**Status:** Closed (`math-kleisli-gate-pipeline`, Queen Wave 6 slot 1).  
...
## Composition (stub types)

The integration witness [`tests/kleisli_gate_pipeline.rs`](../tests/kleisli_gate_pipeline.rs)
...
composition (value threading) but the composed `AdmissibilityResult` reflects the first failure.

## Tests

```bash
export PATH="$HOME/.cargo/bin:$PATH"
cargo test kleisli_gate
```

- `tests/kleisli_gate_pipeline.rs` — pure `kleisli_compose_pair` chain, admissible + inadmissible fixtures.
...
- [`CHANGELOG.md`](../CHANGELOG.md) — `clausius_duhem_violation` landing
