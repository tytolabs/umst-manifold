...
- **HTTP egress retirement (P5):** `GateHttpRuntime::from_defaults()` and `HttpTransitionEvaluator::from_domain_policy_defaults()` are `#[deprecated]` (zero callers). Embedded literals via `default_gate_manifest()` are retired by policy — prefer **`GateManifest::from(&UmstManifest)`** and **`GateHttpRuntime::from_umst_manifest`**. Full table: **`docs/RUNTIME_TOPOLOGY.md`** § HTTP egress retirement.
