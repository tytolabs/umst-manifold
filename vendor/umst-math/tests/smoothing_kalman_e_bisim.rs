...
use umst_math::smoothing::{KalmanSmoother, MetricSmoother};

const EPS: f64 = 1e-9;

fn run_sequence(raw: &[f64]) -> Vec<f64> {
    let mut s = KalmanSmoother::new(0.0);
...
// Same five fixtures as EKF; linear 1D observation model yields the same mean trajectory.
...
fn output_variance_kalman(z: &[f64; 100], q: f64, r: f64) -> f64 {
    let mut s = KalmanSmoother::new_with_q_r(0.0, q, r);
...
fn assert_tuning_helps_kalman(scale: f64, qn: &str, rn: &str) {
    let z = g5_tuning_synth_100(scale);
    let v_u = output_variance_kalman(&z, G5_Q0, G5_R0);
    let q = registry_f64_by_name(qn).expect("registry Q");
    let r = registry_f64_by_name(rn).expect("registry R");
    let v_t = output_variance_kalman(&z, q, r);
    assert!(v_u > 1.0e-20, "non-degenerate output variance (uniform)");
    let imp = (v_u - v_t) / v_u;
    assert!(
        imp + 1e-12 >= TUNING_IMPROV_FRAC,
        "TUI-7b Kalman tuning: expected >= {TUNING_IMPROV_FRAC} var reduction, got {imp}"
    );
}

#[test]
fn kalman_tuning_rcc_g5() {
    assert_tuning_helps_kalman(3.0, "egoff_smoother_q_rcc", "egoff_smoother_r_rcc");
}
#[test]
fn kalman_tuning_mi_g5() {
    assert_tuning_helps_kalman(2.0, "egoff_smoother_q_mi", "egoff_smoother_r_mi");
}
#[test]
fn kalman_tuning_eta_cog_g5() {
    assert_tuning_helps_kalman(10.0, "egoff_smoother_q_eta_cog", "egoff_smoother_r_eta_cog");
}
#[test]
fn kalman_tuning_dignity_g5() {
    assert_tuning_helps_kalman(0.8, "egoff_smoother_q_dignity", "egoff_smoother_r_dignity");
}
#[test]
fn kalman_tuning_landauer_g5() {
    assert_tuning_helps_kalman(
        200.0,
