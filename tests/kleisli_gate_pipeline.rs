...
//! Kleisli gate pipeline composition witness (`math-kleisli-gate-pipeline`).
//!
...
use umst_manifold::gate::{
    kleisli_compose_pair, Admissible, KleisliAdmissibilityResult, KleisliArrow,
...
fn kleisli_gate_pipeline_admissible_composition() {
    ...
    assert!(out.result.admissible, "identity-like propose → zero slack");
