...
use umst_manifold::physics::solve_budget::{q1hex_opts_from_cockpit, CockpitSnapshot, DEFAULT_PCG_MAX_ITER};
...
fn l1_1_pcg_reduction_extreme_magnitude_parity() {
    let n = 512usize;
    let a: Vec<f32> = (0..n).map(|i| (i as f32 + 1.0) * 1e-3 * (1.0 + (i % 7) as f32 * 1e6)).collect();
