...
fn topology_density_diffusion_idempotent_on_uniform_rho() {
    ...
    for e in 0..n { edges.push(e as i64); edges.push(((e + 1) % n) as i64); }
...
fn rheology_step_idempotent_on_quiescent_bingham_equilibrium() {
    use umst_manifold::physics::solvers::BinghamFlowSolver;
    let dev = device();
    let edges_b1 = Tensor::<B, 2, Int>::from_data(Data::new(vec![0i64, 1], Shape::new([2, 1])), &dev);
...
#[cfg(any(feature = "topology-density-evolution", feature = "mechanics-voigt-cauchy"))]
#[test]
fn q1_hex_solve_equilibrium_idempotent_on_zero_load_fixed_bc() {
    ...
    let dev = device();
    let plate = ExtrudedPlateMechanics { nx: 2, ny: 2, nz: 1, dx: 0.1, dy: 0.1, dz: 0.05 };
    let n = plate.n_nodes();
