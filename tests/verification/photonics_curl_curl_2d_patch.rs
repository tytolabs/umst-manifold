...
//! **P4B — uniform 2D brick:** [`photonics_uniform_brick_two_quad_strip_tensors`] + [`PhotonicsSolver::solve_maxwell_curl_curl`].
//! Uses the shipped DEC patch path (not the uniform x-chain TE reduction).
...
use umst_manifold::physics::solvers::photonics::{
    dec_patch_maxwell_natural_matvec_flat, photonics_uniform_brick_two_quad_strip_tensors,
    UNIFORM_BRICK_TWO_QUAD_STRIP_FACE_RANGES,
};
...
#[test]
fn solve_maxwell_uniform_brick_two_quad_strip_residual() {
    let dev = device();
    let cell_h = 0.25_f32;
    let (edges_b1, faces_b2, coords) =
        photonics_uniform_brick_two_quad_strip_tensors::<B>(cell_h, &dev);
    let patch = PhotonicsDecFacesPatch {
        faces_b2: &faces_b2,
        face_column_ranges: &UNIFORM_BRICK_TWO_QUAD_STRIP_FACE_RANGES,
    };
    let n = 6usize;
    ...
    let sol = ps.solve_maxwell_curl_curl(
        ...
        &cg,
        Some(&patch),
    ).expect("PhotonicsSolver::solve_maxwell_curl_curl on two-quad strip patch boundary residual witness (FP §6 Track G photonics)");
    ...
    for r in 0..3 {
        assert!((x[r] - e0[r]).abs() < 1e-3_f32, "gauge pin at node 0");
    }
    for r in 3..dim {
        let br = scale_j * jv[r];
        assert!(
            (y[r] - br).abs() < 9e-3_f32 || (y[r] - br).abs() / br.abs().max(1e-9_f32) < 0.03_f32,
