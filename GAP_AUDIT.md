...
| **8** | Newmark elastodynamics + TopOpt coupling | **Research (`acoustics-newmark`):** dense-reference Newmark; plane-wave **n=100** return-map CI gate plus **n=64** / **n=128** live **`plane_wave_return_map_*_l2_within_two_percent`** brackets (same CFL-scaled recipe; Solver-Status); **n=128** **`undamped_energy_drift_under_half_percent_over_1000_steps`** energy proxy is separate; **plan gaps:** TopOpt coupling; default build may still no-op [`step_wave`](src/physics/solvers/acoustics.rs) without feature | `umst-manifold` | **L** |
...
- [ ] Phase 8: TopOpt coupling with acoustics; return-map suite — **n=100** CI gate, **n=64** / **n=128** live **`plane_wave_return_map_*_l2_within_two_percent`** brackets; Newmark / energy-drift harnesses behind `acoustics-newmark` (Solver-Status acoustics row / **DEFERRAL — Acoustics** for graph-wide / coupling scope beyond the 1-D periodic bar lane).
