...
These items were previously called out as inconsistent or missing; the current `umst-manifold` tree addresses them at **engineering** scope; **default `cargo test` is green** locally, and **`cargo test --features solver-tests`** (run in root CI) is **green** on the verified snapshot — see **Verification: Cargo tests** below.
...
**Disposition (plan vs repo):** Phase **1** mechanics + emergence unit tests are **green** (default + **`solver-tests`**). Phases **2–9** still differ from the bleeding-edge plan in material ways: **default-build stubs**, paths gated on **`solver-experimental`**, missing pieces (e.g. SG, curl–curl), or research-grade scope. The table rows below state **remaining** gaps; several composer refinements are **done** — see **Refinements verified in code**.
...
| **1** | Full 3D Voigt elasticity + edge interpolation per plan | Current solver is bar-network / reduced scope vs prompt’s full \( \sigma = C:\varepsilon \) on general skeleton | `umst-manifold` | **L** |
| **2** | AT2 phase-field + spectral strain split + irreversibility | **Default build:** `update_damage` no-op. **`solver-experimental`:** AT2 relaxation in [`fracture_field.rs`](src/physics/solvers/fracture_field.rs); full plan coupling / 3D generality still **not** done | `umst-manifold` | **L** |
| **3** | Bingham Navier–Stokes + Chorin + thixotropy | `BinghamFlowSolver` MVP / partial; convection and full coupling not plan-complete | `umst-manifold` | **L** |
| **4** | Neural-SIMP + augmented Lagrangian + sensitivity filter | `DensityNet` / `TopologyOptimizer` shell exists; full MBB / crisp projection / filter loop not demonstrated | `umst-manifold` | **M** |
| **4** | `topology_solver` density-on-sheaf evolution | Scaffold only (`physics/solvers/topology_solver.rs`) | `umst-manifold` | **M** |
| **5** | Monolithic THMC Newton + autodiff Jacobian + adaptive \(dt\) | [`ThmcSolver::step`](src/physics/solvers/thmc.rs) runs explicit transport + hydration + quasi-static mechanics with an **outer Newton loop** on \(R_T+R_h\) vs `tol`; **not** a monolithic implicit Jacobian solve or adaptive \(dt\) | `umst-manifold` | **L** |
| **5** | Rheology inside same tick as THMC | Orchestrator docs: Bingham **not** folded into `ThmcSolver::step` yet | `umst-manifold` | **M** |
| **6** | PNP + Poisson + Scharfetter–Gummel | Poisson + diffusion + **drift surrogate** in [`electrochemistry.rs`](src/physics/solvers/electrochemistry.rs); **Scharfetter–Gummel** upwinding **not** implemented | `umst-manifold` | **L** |
| **7** | Frequency-domain Maxwell curl–curl on DEC | `photonics` driver returns input unchanged (stub) | `umst-manifold` | **L** |
| **8** | Newmark elastodynamics + TopOpt coupling | **Default build:** [`step_wave`](src/physics/solvers/acoustics.rs) no-op passthrough. **`solver-experimental`:** implicit Newmark-β tensor step — TopOpt coupling still **not** demonstrated | `umst-manifold` | **L** |
| **9** | Atomistic → continuum upscale (virial / partition functions) | MVP `StatisticalBridge::upscale_potentials` (heuristic scaling + tests) vs rigorous multi-scale bridge | `umst-manifold` | **L** |
...
*Partial refinement implementations (feature-gated) are listed under **Refinements verified in code**; items below remain **plan-complete** gaps.*

- [ ] Phase 2: AT2 phase-field + spectral strain split + irreversibility — replace `PhaseFieldFractureSolver::update_damage` no-op stub.
- [ ] Phase 3: Bingham Navier–Stokes + Chorin + thixotropy to plan completeness (`BinghamFlowSolver` MVP gaps).
- [ ] Phase 4: Neural-SIMP + augmented Lagrangian + sensitivity filter demonstrated; evolve `topology_solver` density-on-sheaf beyond scaffold.
- [ ] Phase 5: Monolithic implicit THMC Newton + autodiff Jacobian + adaptive \(dt\); fold Bingham rheology into `ThmcSolver::step` (orchestrated coupling).

### P4 — Phases 6–9

- [ ] Phase 6: PNP + Poisson + Scharfetter–Gummel — implement `electrochemistry` beyond scaffold.
- [ ] Phase 7: Frequency-domain Maxwell curl–curl on DEC — replace `photonics` input-echo stub.
- [ ] Phase 8: Newmark elastodynamics + TopOpt coupling — implement `acoustics` beyond scaffold.
- [ ] Phase 9: Rigorous atomistic → continuum upscale (virial / partition functions) beyond `StatisticalBridge::upscale_potentials` MVP.
